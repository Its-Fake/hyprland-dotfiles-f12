...
def get_network_status():
    ...
    # 5. Killswitch Status
    # Prüft, ob die nftables-Tabelle 'vpn_killswitch' existiert
    killswitch_check = subprocess.run(["sudo", "nft", "list", "table", "inet", "vpn_killswitch"], 
                                     capture_output=True, text=True)
    status["killswitch_active"] = killswitch_check.returncode == 0
